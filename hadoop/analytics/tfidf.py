...
df_review = sesh.read.option('header', False).option( 'delimiter', '_').schema(schema) \
.csv('hdfs://n1:9000/user/hadoop/reviews/part-m-00000')

cmd =b'/opt/hadoop-3.3.0/bin/hdfs dfs -ls reviews'
files = subprocess.check_output(cmd, shell=True).strip().split(b'\n')

for i in range(len(files)-3):
        df_review_i =  sesh.read.option('header', False).option( 'delimiter', '_').schema(schema) \
                                .csv('hdfs://n1:9000/user/hadoop/reviews/part-m-0000{}'.format((i+1)))
...
tfidf_table.write.format('com.databricks.spark.csv').save('/home/hadoop/tfidf_output')
